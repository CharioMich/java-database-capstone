...
public class Service {
    ...
    public Service(
            ...
            DoctorService doctorService, PatientService
                    patientService) {
    ...
    // Instructions want this method to return a ResponseEntity<>, otherwise it would be more simple and clear to return a boolean value
    ...
    public ResponseEntity<Map<String, String>> validateAdmin(Admin admin) {
        ...
        try {
            ...
            response.put("error", "Admin with username: " + admin.getUsername() + " not found.");
            return new ResponseEntity<>(response, HttpStatus.UNAUTHORIZED);
        } catch (Exception e) {
            response.put("error", "Something went wrong while validating admin.");
    ...
    // THIS METHOD IS ALREADY IMPLEMENTED IN THE DoctorService CLASS
    ...
    public int validateAppointment(Appointment appointment) {
        try {
            Doctor storedDoctor = doctorRepository.findById(appointment.getDoctor().getId())
                    .orElseThrow(() -> new EntityNotFoundException("Doctor not found for specific appointment"));

            List<String> availableTimes = doctorService.getDoctorAvailability(storedDoctor.getId(), appointment.getAppointmentDate());
            // Unavailable time
            if (!availableTimes.contains(appointment.getAppointmentTimeOnly().toString()))
                return 0;
            // Valid appointment
            return 1;
        } catch (EntityNotFoundException e) {
    ...
    public boolean validatePatient(Patient patient) {
        try {
            if (patientRepository.findByEmailOrPhone(patient.getEmail(), patient.getPhone()).isPresent())
                throw new EntityExistsException("Patient with current phone or email already exists.");
            return true;
    ...
    public ResponseEntity<Map<String, String>> validatePatientLogin(Login login) {
        ...
        try {
            ...
            response.put("error", "Patient with email: " + login.getIdentifier() + " not found.");
            return new ResponseEntity<>(response, HttpStatus.UNAUTHORIZED);
        } catch (Exception e) {
            response.put("error", "Something went wrong while validating patient login.");
            return new ResponseEntity<>(response, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> filterPatient(String condition, String doctorName, String token) {
        ...
        try {
            ...
            // Determine which filters are provided
