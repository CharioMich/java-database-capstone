...
public class TokenService {
    ...
    // Retrieves value from application.properties
    @Value("${jwt.secret}")
    ...
    public String generateToken(String identifier) {
        ...
        Instant expiration = now.plus(7, ChronoUnit.DAYS); // or configurable via application.properties
        ...
        return Jwts.builder()
                ...
                .expiration(Date.from(expiration))   // Token expiration
                .signWith(key)                          // Modern API infers HS256 automatically
                .compact();
    }
    ...
    public String extractIdentifier(String token) {
        // Parse and validate the JWT using the same signing key
        Claims claims = Jwts.parser()
                .verifyWith(getSigningKey())   // Verifies the token signature
                .build()
                .parseSignedClaims(token)             // Parses the token and extracts claims
                .getPayload();

        // Return the subject (identifier: email or username)
